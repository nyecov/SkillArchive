...
- name: Value Stream Mapping (VSM)
  path: ../vsm/SKILL.md
- name: Shisa Kanko Declaration Template
  path: ./templates/shisa-kanko-declaration.md
requires:
- kyt
- genchi-genbutsu
- jidoka
...
- **Action:** Output a `[TARGET_ISOLATION]` block containing the file absolute path, exact line ranges, and the verified code snippet.
- **Constraint:** If the snippet cannot be perfectly matched via a `read_file` or `view_file` check, you MUST NOT proceed to execution.
- **Integration:** Directly feeds into the **Poka-yoke** declaration.
...
- **Action:** Activate the **KYT** skill to identify hazards and establishing countermeasures.
- **Constraint:** NEVER execute a plan identified as "High Risk" without a verified Round 4 Action Target from the KYT pass.
- **Integration:** Chained interlock between **KYT** (Risk) and **Shisa Kanko** (Execution).
...
- **Action:** Output a strictly formatted `[LOGIC_DECLARATION]` block defining the intent, success criteria, and verification method.
- **Constraint:** All execution MUST be preceded by this declaration. You MUST prioritize writing/running verification scripts before final implementation.
- **Integration:** Bridges to **Test-Driven Development (TDD)** ensuring tests are ground truth.
...
- **Constraint:** A task is not complete until it has been empirically verified by system output, not just agent assumption.
- **Integration:** Final quality gate before task completion.
