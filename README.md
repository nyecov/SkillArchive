...
| **[Gemba (The Real Place)](skills/gemba/SKILL.md)** | Use to establish a factual baseline of the codebase and navigate unfamiliar systems. Mandates direct observation befo... | `context` `methodology` `research` `lean` `TPS` `exploration` `codebase` `search` `engineering` |
...
| Skill | Description | Tags |
|-------|-------------|------|
| **[Agile Release Gate](skills/agile-release-gate/SKILL.md)** | Use when a feature or epic is "code complete" and approaching release. Governs the pre-release audit checklist, trace... | `agile` `release` `engineering` `safety` `methodology` |
| **[Architecture & Anchoring](skills/architectural-anchoring/SKILL.md)** | Use when making architectural decisions, enforcing layer boundaries, or when AI-suggested changes conflict with estab... | `design` `architecture` `agile` `clean-architecture` `methodology` |
| **[Code Review](skills/code-review/SKILL.md)** | Use when performing a structured code review on a PR, diff, or changeset. Handles change detection, impact analysis, ... | `engineering` `code-review` `quality` `testing` |
| **[Isolate (Root Cause Isolation)](skills/root-cause-isolation/SKILL.md)** | Use when a bug is reported, unexpected behavior occurs, or a fix attempt has failed. Handles 5-step hypothesis-driven... | `debugging` `search` `testing` `engineering` |
...
| **[Refactor Safely](skills/refactor-safely/SKILL.md)** | Use when planning or executing any refactor — from renaming a symbol to restructuring a layer boundary. Enforces impa... | `engineering` `refactoring` `safety` `methodology` |
...
| Skill | Description | Tags |
|-------|-------------|------|
| **[Agile Backlog Authoring](skills/agile-backlog/SKILL.md)** | Use when creating, editing, refining, or reviewing Agile artifacts: Initiatives, Epics, User Stories, Subtasks, Issue... | `agile` `backlog` `methodology` `stories` `artifacts` |
| **[Agile Testing Workflow](skills/agile-testing/SKILL.md)** | Use when authoring Test Plans (per Epic), Test Designs (per Story AC), or Test Executions with evidence. Handles UI, ... | `agile` `testing` `methodology` `regression` `ui-testing` |
...
| **[Integrated Agile SDLC](skills/agile-sdlc/SKILL.md)** | Use when orchestrating the full Software Development Lifecycle (SDLC) across Agile, TDD, and testing stages. Handles ... | `agile` `sdlc` `methodology` `tdd` `testing` |
...
| **[Spec Mapping: Surgical Carving & Convergence Workflow](skills/spec-mapping/SKILL.md)** | Use when mapping a spec document into the SDLC backlog as epics + stories with a coverage ledger. CLI-neutral, append... | `agile` `spec` `mapping` `backlog` `methodology` |
...
| **[agile-hierarchy-validator](tools/agile-hierarchy-validator)** |  |
| **[agile-turn-commit](tools/agile-turn-commit)** |  |
| **[audit-go](tools/audit-go)** |  |
| **[audit-python](tools/audit-python)** |  |
| **[backlog-tools](tools/backlog-tools)** |  |
| **[chaos-tester](tools/chaos-tester)** |  |
...
| **[spec-gap-tools](tools/spec-gap-tools)** |  |
| **[tdd-phase-verifier](tools/tdd-phase-verifier)** |  |
| **[triage-validator](tools/triage-validator)** |  |
