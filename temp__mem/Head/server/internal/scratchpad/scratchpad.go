...
func HandleLogSessionFinding(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("ToolError: Failed to open tmp file: %v", err)), nil
	}
	defer f.Close()

	if _, err := f.Write(bytes); err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("ToolError: Failed to write to tmp file: %v", err)), nil
	}
	if err := f.Sync(); err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("ToolError: Failed to sync file: %v", err)), nil
	}
	f.Close()
